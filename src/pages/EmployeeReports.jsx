...
import { FaUser, FaChartBar, FaChartLine, FaCheckCircle, FaExclamationCircle, FaRupeeSign, FaCalendarDay, FaCalendarWeek, FaCalendarAlt, FaHistory, FaFilter, FaTimes } from "react-icons/fa";
...
const EmployeeReports = () => {
  ...
  const { token, admin } = useAuth(); // admin for SuperAdmin, user for Branch Panel (using authContext mapping)
    ...
    <div className="p-6 animate-fade-in max-w-7xl mx-auto">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 gap-4">
        <div>
          ...
          <p className="text-sm" style={{ color: themeColors.textSecondary }}>
            Detailed sales performance, deal values, and metrics for specific sales executive.
          </p>
        </div>
      </div>

      {/* Selector Box */}
      <div className="mb-6 p-6 rounded-xl border shadow-sm" style={{ backgroundColor: themeColors.surface, borderColor: themeColors.border }}>
      ...
      {isFetching ? (
        <div className="py-20 flex flex-col items-center justify-center">
          ...
          <p className="mt-4 font-medium" style={{ color: themeColors.textSecondary }}>Generating employee reports...</p>
        </div>
      ) : currentData ? (
        <>
          {/* Lead KPI Cards */}
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
            ...
            >
              <div className="flex justify-between items-start">
                <div>
                  <p className="text-sm font-medium mb-1" style={{ color: themeColors.textSecondary }}>Total Assigned Leads</p>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="rounded-xl p-6 shadow-sm border h-full" style={{ backgroundColor: themeColors.surface, borderColor: themeColors.border }}>
              <h3 className="text-lg font-bold mb-2 flex items-center gap-2" style={{ color: themeColors.text }}>Lead Status</h3>
              {Object.keys(currentData.statusBreakdown || {}).length > 0 ? (
                <>
                  <HighchartsReact highcharts={Highcharts} options={getPieChartOptions(currentData.statusBreakdown)} />
        ...
        <div className="text-center py-20 rounded-xl border border-dashed" style={{ borderColor: themeColors.border }}>
          <p className="text-lg font-medium" style={{ color: themeColors.textSecondary }}>Select a sales employee to view their detailed performance report.</p>
        </div>
      )}
...
export default memo(EmployeeReports);
